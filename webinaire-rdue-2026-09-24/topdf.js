...
(async()=>{ const b=await chromium.launch({executablePath:'/opt/pw-browsers/chromium/chrome-linux/chrome'}).catch(async e=>{ const fs=require('fs'); const dirs=fs.readdirSync('/opt/pw-browsers').filter(d=>d.startsWith('chromium-')); const p='/opt/pw-browsers/'+dirs[0]+'/chrome-linux/chrome'; return chromium.launch({executablePath:p}); });
 const pg=await b.newPage(); await pg.goto('file://'+path.resolve('guide_print.html'),{waitUntil:'load'});
 await pg.pdf({path:'GUIDE_RDUE_cas_pratiques.pdf',format:'A4',printBackground:true,displayHeaderFooter:true,
   headerTemplate:'<div style="font-size:7.5pt;color:#8A98A8;width:100%;padding:0 18mm;font-family:Carlito,Calibri,Arial">Guide pratique RDUE · CCI Lyon Métropole Saint-Étienne Roanne · CF² Douane</div>',
